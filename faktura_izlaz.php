                        ...
                        <table border='1' bordercolor='gainsboro' class='table table-hover' id='sample1'>
                            <thead>	
                                <tr>
                                    <th>Redni broj</th>
                                    <th>Naziv</th>
                                    <th>Kolicina ukupno</th>
                                    <th>Komada ukupno</th>
                                    <th>Ukupan iznos kolicina</th>
                                    <th>Ukupan iznos komad</th>
                                </tr>
                            </thead>
                <?php
                        ...
                        $upitFaktura="Select *, SUM(kolicina_izlaz) as kolicina_ukupno, SUM(ukupna_cena_kolicina_izlaz) as cena_ulaz_ukupno, SUM(ukupna_cena_komad_izlaz) as cena_komad_ukupno from izlaz_lekova u LEFT JOIN lekovi l on u.lek_id = l.id_lek Where u.datum_izlaza BETWEEN ? and ? GROUP BY id_lek";
                        ...
                        foreach($resultFacture->fetchAll() as $row){
                            $counter++;
                            $iznos= $row['cena_ulaz_ukupno']+$row['cena_komad_ukupno'];
                            $mojIznos+=$iznos;
                        ?>
                            <tr>
                                ...
                                <td><?php echo $row['komada_izlaz'];?></td>
                                <td><?php echo $row['cena_ulaz_ukupno'];?></td>
                                <td><?php echo $row['cena_komad_ukupno'];?></td>
                            </tr>
                    <?php
                        }
                       ?>
                            <tr>
                                <td  colspan='6'>Ukupno:<br/><?php echo $mojIznos.".00RSD";?></td>
                            </tr>
                    <?php      
